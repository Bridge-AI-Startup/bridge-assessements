...
describe("orderChecksForIsolation", () => {
  it("runs read-only checks before mutating checks", () => {
    ...
    // Adding a note and the refresh-persistence check both change state, so
    // they run after the read-only welcome check, in original order — the
    // persistence check must not run before the note it reads back exists.
