...
└── scripts/               # Utility/migration scripts
    ...
    ├── seedPerfectDemoSubmission.ts # Fully-populated demo submission ("Jordan Avery") on the Standup Board assessment under saaz.m@icloud.com + demo@bridgeai-demo.com: 30-min stitched screen recording (S3), workflow-capture events + persisted episodes, companion voice transcript, evaluation report (rubric 9/9/9/**3** — the low "edits agent code" score is deliberate demo contrast, paired with a contradicted spoken claim in the communication card; clean capture integrity), behavioral report 6/6 with real screenshots, uploaded code archive (demos/standup-board-demo), finalized+verified runtime setup. Dummy data, but metrics/score/integrity computed via the real computeMetrics/computeBehavioralScore/assessCaptureIntegrity. Needs server/storage/demo/standup-board-playback.webm (or DEMO_VIDEO_PATH). Idempotent per owner. Grading screenshots live in LOCAL grading storage only (server/storage/grading) — they 404 from Render; everything else (video, voice, archive) is in S3 and works everywhere
...
- `PATCH /:id` -- Update assessment (auth required); optional `evidenceMode` (`both` / `none`; leftover `workflow` / `screen` still accepted); optional `pinned` (bool — Home pin-to-top)
...
- `GET /tester` plus `GET /dev/data`, `GET /dev/file`, `GET /dev/stage/:stage`, `POST /dev/run/:stage` -- **the capture lab, development only** (not mounted when `NODE_ENV=production`, and every handler re-checks — these return capture tokens, raw prompts and code, and can spend money on model calls). Open `http://localhost:5050/api/workflow-capture/tester`, run `node capture-kit/setup.js --local` in a scratch repo, and work in Claude Code: the page shows **every byte captured and every inference drawn from it**, live. Six tabs — Session (setup command, session document, consent/environment, capture integrity, linked submission/proctoring, env readiness), Raw stream (every event with its full payload, seq, cwd, branch, truncation), Timeline (the built `TranscriptEvent[]` the evaluator reads, plus the voice transcript), Code (file state with contents), AI pipeline, Recording — over a persistent video dock whose scrubber carries the screen band and one tick per event. **The split that matters:** deterministic work (metrics, timeline, capture integrity, video offsets) is recomputed on every 2s poll, and anything costing a model call is a *stage* — started explicitly, run in the background, cached with status/timing so the page shows "running… 14s" instead of hanging. Stages are `screen` (Gemini classification), `episodes`, `evaluate` (validate → ground → evaluate → citation validation → communication → capture integrity, against criteria typed in the page or taken from the linked assessment), `communication`, `agentContext` (the bundle `get_candidate_context` would return), and `director` (what the interviewer would ask right now, with the exact prompt payload shown — the prompt-iteration surface, and it never publishes a briefing). **Run full pipeline** runs them in production order (screen first, so episodes see the band); **auto-run director** re-decides every 30s, the real loop's cadence. A session with no submission behind it gets a synthetic context bundle built from the capture stream and the lab's default criteria, so every stage is exercisable without a whole assessment. Stage results are mirrored to `server/storage/devlab/` — the lab is used with nodemon running, and an in-memory-only cache throws away a minute of model calls on every save. Page markup lives in [`services/workflowCapture/testerPage.ts`](server/src/services/workflowCapture/testerPage.ts), its browser code in [`testerPageScript.ts`](server/src/services/workflowCapture/testerPageScript.ts) (plain string — no backticks, no `${}`, guarded by `server/test/unit/captureLabPage.test.ts`, because one stray quote renders a blank page that looks like a dead server), data and stage running in [`devLab.ts`](server/src/services/workflowCapture/devLab.ts)
...
7. **Scoring**: Combined employer/leaderboard score from available signals — Process (how-they-worked rubric via `evaluationReport`) and Code (E2B product-check pass rate). Deprecated Trace / LLM-workflow scoring was removed.
