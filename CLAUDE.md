...
- **Delete a build (`DeleteBuildModal`):** `shorts/client/src/components/DeleteBuildModal.jsx`. Owners can take a build down from My builds, the gallery "Your submissions" strip, and the Submission page when `isMine`. Confirm copy is one sentence (leaves the gallery and the ranking; can't be undone). Same server cleanup as admin delete. The submit dialog names the 3-per-round cap; a fourth submit is **409** `{ code: "submission_limit" }` with copy **"You ran out of builds for the week."** — it does not tell them to delete one to free a slot. Signed-in `smahadkar@ucsd.edu` skips the cap (`unlimitedSubmit.ts`).
...
- `POST /session/:id/pause` -- Pause E2B sandbox while user leaves Build (`{ anonymousId }`); skipped while `currentTurn.status === "running"` so an in-flight `claude -p` is not killed. Session stays active until the round ends.
