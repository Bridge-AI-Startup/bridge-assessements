...
**Grading a workflow submission.** When `evidenceMode` resolves to `workflow`/`both`, `ensureProctoringTranscriptAndEvaluate` skips the video transcript and calls [`evaluateWorkflowSession`](server/src/services/workflowCapture/evaluate.ts) instead, which builds the timeline and runs the **existing** `evaluateTranscript` orchestrator (grounder → per-criterion evaluator → session summary). Same `evaluationReport` shape is stored on the submission, so dashboard, scoring and the interview agent need no changes; the report additionally carries `workflowMetrics` and an `evidenceIntegrity` block. Without this, a workflow-mode submission would produce **no score at all** — worse than one nobody can look at yet.
...
11. **Employer reviews**: SubmissionsDashboard → stats, filtering, dropoff analysis, evaluation modal (Final code / Recording & rubric / Agent), interview transcript + analysis
