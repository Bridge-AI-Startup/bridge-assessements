...
- **Out of time (`BuildTimeModal`):** two clock pop-ups sharing the same shell as the credits one. A **warning** fires at `TIME_WARNING_MS` (60s) before `expiresAt` — Submit build / Keep building — and is suppressed if the submit dialog is already open. At zero, **Time's up** replaces the old bare expired screen: Submit build / Try again (starts a *new* session with a new clock) / Cancel, with the remaining submit window counted down. The page only falls back to the full-screen expired state once the grace is spent. Building really does stop at `expiresAt` — chat is disabled client-side and `runClaudePrintPrompt` rejects with `session_expired` (serverless already did), so the grace never doubles as extra build time.
- **Submit grace (`SHORTS_SUBMIT_GRACE_SECONDS`, default 120):** `POST /submit` accepts a timed-out session for this long after `expiresAt` (`isWithinSubmitGrace` in `sessionPersist.ts`); status may already be `expired`, which the gate allows. Session reaping is delayed by the same window so the sandbox survives long enough to snapshot, and an E2B submit whose box is already gone falls back to the persisted `workspaceSnapshot` — safe because building stopped at expiry, so the snapshot *is* the final state. The session payload carries `submitGraceSeconds` so the client can count it down. Set to `0` to disable.
- **Shorts nav (`ShortsHeader`):** inline sections + account dropdown above `sm`; below it both collapse into one hamburger dropdown holding the same links plus Build and the account actions. The page CTA pill stays visible at every width.
## Ports and URLs
...
- `SHORTS_SUBMIT_GRACE_SECONDS` -- Window after `expiresAt` in which a timed-out build can still be **submitted** (default: `120`; `0` disables). Submit-only — chat turns are refused at `expiresAt`. Session reaping waits out this window so the sandbox is still there to snapshot
...
- `POST /session/:id/submit-hold` -- Stop the submit clock (`{ anonymousId }`); called when the submit dialog opens. Returns `{ heldAt, submitDeadline }`; **400** `session_expired` if the build is already past the grace window. Does not extend build time
...
Fields: `anonymousId` (indexed), `challengeSlug`, `challengeDate` (`YYYY-MM-DD`), `status` (`provisioning`/`active`/`failed`/`expired`/`submitted`), `makeMode` (optional `e2b`/`serverless`; stamped at creation, unset → treated as `e2b`), `e2bSandboxId` (absent for serverless), `previewUrl` (serverless: backend `/session/:id/preview`; E2B: sandbox URL), `tokenBudget`, `tokensUsed` (default 0), `llmProxyToken` (Bearer for Messages proxy; never returned to browser), `llmCalls` (optional counter), `startedAt`, `expiresAt` (wall-clock build limit: `min(startedAt + timeLimit, end of UTC day)`), `chatMessages[]` `{ role, text, createdAt }` (persisted Claude chat), `workspaceSnapshot[]` `{ path, content }` + `workspaceSnapshotAt` (E2B: file backup for sandbox recreate; serverless: the authoritative generated file(s)), `sandboxPaused` (bool; paused sessions excluded from concurrent cap), `submitHoldAt` (submit-clock hold, see below), `error` (optional)
...
**Submit hold (`submitHoldAt`):** opening the submit dialog stops the *submit* clock — `POST /session/:id/submit-hold` stamps `submitHoldAt`, and `POST /submit` then accepts the build for 15 minutes from that stamp (`isWithinSubmitHold` in `sessionPersist.ts`), on top of the shorter `SHORTS_SUBMIT_GRACE_SECONDS` window. This exists because naming a build — and especially creating an account or going through a Google popup — can easily outlast the grace window, and losing a finished build to that is the worst failure the product has. It never buys build time: `expiresAt` is untouched, so chat turns and file writes still stop exactly when they did. The hold is re-stamped on every open, and refused outright once the build is already past grace (an expired session cannot be revived by opening the dialog). The Build page freezes the displayed countdown while the dialog is open and skips the expiry teardown, so the dialog can never be unmounted out from under a signup.
